...
async function checkAdmin() {
  const adminRef = doc(db, "admins", String(user.id));
...
async function createTask() {
  ...
  const timer = Number(document.getElementById("taskTimer").value);
  const limit = Number(document.getElementById("taskLimit").value);
